...
export function ZonasPage() {
  ...
  const salvar = async () => {
    ...
    const r = editando
      ? await accessApi.put(`/access/zonas/${editando.id}`, corpo)
      : await accessApi.post("/access/zonas", corpo);
    setSalvando(false);
    if (!r.ok) {
      setErroForm(r.erro);
      return;
    ...
    setFeedback({ tipo: "sucesso", mensagem: editando ? "Zona atualizada." : "Zona cadastrada." });
    carregar(pagina);
  };
  ...
  const excluir = async () => {
    setExcluindo(true);
    setErroExcluir("");
    const r = await accessApi.delete(`/access/zonas/${excluirId}`);
    setExcluindo(false);
    if (!r.ok) {
    ...
    setExcluirId(null);
    setFeedback({ tipo: "sucesso", mensagem: "Zona excluída." });
    carregar(pagina);
  };
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Agrupamento de salas dentro da unidade</p>
        </div>
        <button className="btn btn-brand" onClick={abrirNovo}>
          <Icon name="Plus" size={14} /> Nova Zona
        </button>
      </div>

      <Aviso tipo="info">
        A zona é o nível intermediário entre a unidade e a sala — costuma corresponder a um bloco,
        a um andar ou a um segmento (Infantil, Fundamental I). Toda sala pertence a uma zona.
      </Aviso>
        ...
        <table className="data-table">
          ...
          <tbody>
            <LinhasEstado
              ...
              icone="Map"
              tituloVazio="Nenhuma zona cadastrada"
              textoVazio="Crie ao menos uma zona para poder cadastrar salas."
      ...
      <Modal
        ...
        title={editando ? "Editar Zona" : "Nova Zona"}
        footer={
          <>
        ...
        <div className="form-grid">
          ...
          <div className="form-field">
            ...
            <input
              ...
              value={form.nome}
              onChange={campo("nome")}
              placeholder="Ex.: Bloco B — Fundamental I"
            />
            {erros.nome && <span className="form-error">{erros.nome}</span>}
          </div>
          <div className="form-field">
            <label className="form-label">Descrição</label>
            <textarea className="form-textarea" rows={3} value={form.descricao} onChange={campo("descricao")} />
          </div>
        </div>
      </Modal>
      ...
      >
        As salas vinculadas a esta zona precisarão ser reclassificadas. Confirma a exclusão?
      </ConfirmarModal>
    </div>
  );
}
