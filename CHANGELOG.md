...
### Melhorias
- **O painel SaaS mostra a carteira, não uma tabela.** A Visão geral abre com saudação e quantas redes estão no ar; "Carteira" traz cadastradas, ativas, suspensas e sem módulo numa linha só; "Cobertura por módulo" diz quantas redes contratam cada bloco — a leitura de onde ainda há o que oferecer. A lista de redes virou cards com busca por nome ou CNPJ e filtros (Todas, Ativas, Suspensas, Sem módulo).
- **A régua de módulos.** Cada rede e cada plano mostram cinco casas, uma por módulo, preenchidas conforme o contrato. Dá para ver de relance quem tem o quê sem ler lista nenhuma. Em cards estreitos a régua vira ícones; na lista de recentes, cinco traços.
